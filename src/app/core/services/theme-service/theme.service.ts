...
export class ThemeService {
  ...
  constructor(private http: HttpClient) {
    ...
    this.loadThemes(savedDarkMode ? 'dark' : 'light');
  }

  /**
   * Load themes from the server (themes.json).
   * If a preferred themeName is provided (e.g., 'dark' or 'light'), 
   * we try to apply that after themes are loaded.
   */
  loadThemes(preferredThemeName?: string): void {
    this.http.get<Theme[]>('/themes.json').subscribe(
      (themes) => {
        this.themes = themes;
        console.log('[ThemeService] Themes loaded:', this.themes);

        // Apply the preferred theme if available, else apply first theme
        if (preferredThemeName && this.themes.find((t) => t.name === preferredThemeName)) {
          this.applyThemeByName(preferredThemeName);
        } else if (!this.currentTheme && themes.length > 0) {
          this.applyThemeByName(themes[0].name); // Apply the first theme if none is set
        }
      },
      (error) => {
  ...
  applyThemeByName(themeName: string): void {
    ...
    if (theme) {
      ...
      // If we are applying "dark" or "light", update the document class and localStorage
      if (themeName === 'dark') {
  ...
  applyThemeSmoothByName(themeName: string, duration: number = 200): void {
    ...
    // Handle document class and localStorage as well
    if (themeName === 'dark') {
      document.documentElement.classList.add('dark');
      localStorage.setItem('darkMode', 'true');
    } else if (themeName === 'light') {
   ...
   * Assumes "light" and "dark" themes exist in this.themes.
   */
  toggleTheme(): void {
    const isCurrentlyDark = localStorage.getItem('darkMode') === 'true';
    const newThemeName = isCurrentlyDark ? 'light' : 'dark';
    this.applyThemeByName(newThemeName);
    console.log('Theme toggled:', newThemeName);
  }
